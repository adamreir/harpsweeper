...
def permute_conf(conf_lists, T, perm):
    """Permutes the columns such that they are the order usually displayed in sheet music."""
    for t in range(T):
        conf_lists[t] = conf_lists[t][:, perm]  # +1
    return conf_lists

def repr_notes(notes, note_map):
    """takes a set of notes in integer notation. Displays a sorted list of note names"""
    notes = notes.copy()
